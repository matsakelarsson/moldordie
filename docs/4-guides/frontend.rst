...
- `django-cotton`_ compiles ``<c-...>`` components, and the project's UI library supplies them: nine components under ``templates/cotton/ui/``, the stylesheets under ``static/css/ui/``, the ``ui`` app with the filters that write a component's attributes and URLs, the colour palettes and the theme stylesheet. Its contract for downstream developers is the generated project's ``docs/frontend.rst``; the decisions are in ``docs/adr/0009`` and ``docs/adr/0010``.
