...
- `<project_slug>/identity/` — With `rest_api=Django Ninja` and a provider, the app behind the single-page application's login: `base.py` configures allauth's headless API (app client only, JWT strategy, HS256 with `DJANGO_HEADLESS_JWT_PRIVATE_KEY` drawn per environment by the secrets table, stateful validation, refresh rotation) and CORS for `DJANGO_FRONTEND_ORIGINS`; `identity/apps.py` refuses to start with an empty key or a non-positive lifetime (`validate_token_settings`, also run on the loaded production settings by `tests/test_production_settings.py`); `identity/auth.py` holds the Ninja policies (`user_auth`, set globally by `config/api.py`: an app-issued JWT whenever an `Authorization` header is present, else Ninja's session authenticator with its CSRF check); `identity/frontend.py` normalises an origin for `identity/checks.py` (the frontend URL must be a configured origin) and for the account adapter's `is_safe_url`, which accepts an external return destination on a configured frontend origin only; `HEADLESS_FRONTEND_URLS` in `base.py` is the five-path contract with the SPA; the `revoke_jwt_sessions` command deletes the sessions behind app-issued tokens after a key rotation (the runbook is in the generated `docs/authentication.rst`); calling services are `ServiceRegistration` rows (`identity/models.py`, admin with the subject read-only after creation) verified by `identity/verification.py` (PyJWT, RS256 fixed, the provider's rules: Entra's tenant, `idtyp`, role and `oid`, or Google's issuer allowlist, audience and `sub`; the `DiscoveredKeys` factory doing discovery at first use and retrying a failed one on the next lookup; fixed rejection codes in the logs, expiry at info, the rest at warning with identical warnings collapsed per minute) behind the `service_auth` and `either_auth` policies and the `/api/principal/` example (`identity/api.py`); `ServiceRegistration.has_perm` checks the enabled flag and the full `app_label.codename`, and `identity/permissions.py` holds `require_permission`, the 403 of a route under `either_auth`, distinct from the policies' 401; `identity/tests/headless.py` drives the app client and `identity/tests/services.py` mints RS256-signed service tokens for the tests
