...
- `<project_slug>/ui/` — The UI library's Python side: `templatetags/ui.py` with the `ui_attrs`, `ui_attr` and `ui_url` filters over the contracts in `attrs.py` (forwarded attributes: names, booleans, escaping, refused values) and `links.py` (the `navigation` and `local` URL policies); `contrast.py` (WCAG arithmetic), `palettes.py` (the 26 colour tokens, the adjacency table `PAIRS`, the blue, teal and violet palettes), `themes.py` (pure `resolve`, `validate`, `render_stylesheet`; `resolve_theme(request)` as the extension point), `views.py` serving `/ui/theme.css` (`private, no-store`, no transaction), `context_processors.py` exposing `ui_theme`, `checks.py` (`ui.E001`, `ui.E002`); `ui/tests/` with file-backed fixtures under `ui/tests/templates/` so the compiler and loader run on every component test, and the isolation test that names the setting the installed Cotton honours. The generated `docs/frontend.rst` is the library's contract for downstream developers
...
- `<project_slug>/templates/` — Semantic HTML styled by the vendored Pico CSS (`<project_slug>/static/vendor/pico/`, pinned with SHA-256 metadata), with the UI library's `static/css/ui/tokens.css` and the theme stylesheet loaded after it and `data-ui-mode` on the root element when a mode is forced; htmx is loaded through django-htmx's `{% htmx_script %}` (with the `hx-ws` extension when `realtime=channels`). htmx fragments are `{% partialdef %}` partials inside the page template, selected with `htmx_partial` on the view. No Node.js, Bootstrap or asset pipeline.
