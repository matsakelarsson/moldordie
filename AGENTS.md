...
- `<project_slug>/ui/` — The UI library's Python side: `templatetags/ui.py` with the `ui_attrs`, `ui_attr` and `ui_url` filters over the contracts in `attrs.py` (forwarded attributes: names, booleans, escaping, refused values) and `links.py` (the `navigation` and `local` URL policies), and `ui_label` for the field component; `contrast.py` (WCAG arithmetic), `palettes.py` (the 26 colour tokens, the adjacency table `PAIRS`, the blue, teal and violet palettes), `themes.py` (pure `resolve`, `validate`, `render_stylesheet`; `resolve_theme(request)` as the extension point), `views.py` serving `/ui/theme.css` (`private, no-store`, no transaction), `context_processors.py` exposing `ui_theme`, `checks.py` (`ui.E001`, `ui.E002`); `ui/tests/` with file-backed fixtures under `ui/tests/templates/` so the compiler and loader run on every component test (`test_components.py`, `test_field.py` with `forms.py`, `test_allauth.py`), `test_stylesheets.py` (every token the stylesheets read is declared or a palette colour, and they hold no literal colour), and the isolation test that names the setting the installed Cotton honours. The generated `docs/frontend.rst` is the library's contract for downstream developers
